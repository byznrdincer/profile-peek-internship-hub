...
const PersonalInfoSection = ({ formData, setFormData, graduationYearOptions }: PersonalInfoSectionProps) => {
  ...
  const addWebsiteUrl = () => {
    const newUrls = [...localFormData.multiple_website_urls, ""];
    handleInputChange('multiple_website_urls', newUrls);
  };

  const updateWebsiteUrl = (index: number, value: string) => {
    const newUrls = localFormData.multiple_website_urls.map((url, i) => 
      i === index ? value : url
    );
    handleInputChange('multiple_website_urls', newUrls);
  };
  ...
  const removeWebsiteUrl = (index: number) => {
    const newUrls = localFormData.multiple_website_urls.filter((_, i) => i !== index);
    handleInputChange('multiple_website_urls', newUrls);
  };
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <Select 
              value={localFormData.graduation_year} 
        ...
        <div className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                id="github_url"
                value={localFormData.github_url}
          ...
          <div>
            ...
            {localFormData.multiple_website_urls.length === 0 && (
            ...
            <div className="space-y-2">
              {localFormData.multiple_website_urls.map((url, index) => (
